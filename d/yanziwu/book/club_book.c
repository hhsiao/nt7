...
void create()
{
        set_name(HIM"慕容世家"HIR"長兵篇"HIC"棍章"NOR, ({"club book","book"}));
        set_weight(500);
        set("unit", "冊");
        set("long","這是慕容世家的藏書中的"HIR"長兵篇"NOR"之"HIC"棍章"NOR"，包含了普天下所有門派的棍法精要。\n想修絕世武功，就好好看(kan)吧。\n");
        set("value", 1);
        set("material", "paper");
        setup();
}
int do_study(string arg)
{
        ...
        int level = me->query_skill("club",1);
        ...
        me->improve_skill("club", (int)me->query_skill("literate",1)/5+1); 
        write("你研讀了有關基本棍法的技巧，似乎有點心得。\n");
        return 1;
}