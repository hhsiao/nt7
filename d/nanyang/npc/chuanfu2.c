...
int accept_object(object who, object ob)
{
            ...
            if( query("money_id", ob) && ob->value() >= 1000 )
        {
                ...
                who->move("/d/nanyang/hanshuis");
                if (sizeof(obs)) obs->move("/d/nanyang/hanshuis");
...
void goto_peer(object ob, object *obs)
{
        if (arrayp(obs) && sizeof(obs))
        {
                obs -= ({ 0 });
                if (sizeof(obs))
                        obs->move("/d/nanyang/hanshui1");
        }

        if (! objectp(ob) || environment(ob) != find_object("/d/nanyang/hanshuis"))
                return;

           tell_object(ob, "船終於到了對岸。你走下船來，船上的活計將你的貨都拉了下來。\n");
           ob->move("/d/nanyang/hanshui1");
}
