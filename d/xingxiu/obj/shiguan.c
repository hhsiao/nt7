...
void over_liandu(object ob) {
    ...
    if(objectp(corp = present("corpse", ob))&&!userp(corp))
    {
        if(query_temp("liandued", ob) )
            message_vision("\n$N"RED"裡腐爛的屍體猶如一隻洩了氣的皮球一樣，慢慢開始萎縮，最後只剩下一小片血跡在棺底！\n"NOR, ob);
...
int start_liandu(object me) {
    ...
    case 3:
        ...
        break;
    case 5:
        write(HIY"腐屍內的毒素開始緩緩聚於一點，並順著你的手掌向你侵來。\n"NOR);
        break;
    case 11:
    ...
    case 51:
        ...
        break;
    case 61:
        write(BLU"入任脈，起於中極之下，以上毛際，循腹裡，上關元，至咽喉，上頤循面，入目絡舌！\n"NOR);
        break;
    case 71:
    ...
    if(query("neili", me)<0 || query("jingli", me)<0 )
    {
        ...
        tell_room(environment(me), GRN"突然間，你看見"+me->name() + "全身發顫，一股青氣竟然從石棺裡自下而上直衝到"+me->name() + "的前胸！\n"NOR, ({ me }));
        set("neili", 0, me);
