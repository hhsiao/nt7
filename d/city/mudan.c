...
create_food(string nmstr, string* idlist, string longstr)
{
    ...
    if (longstr)
        set("long", @LONG
進門繞過一道淡紅綢屏風，迎面牆上掛著一副『牡丹爭豔』圖，
牡丹是中國國花，素以富貴著稱。圖側對聯： 幽徑天姿呈獨秀，古園
國色冠群芳。襯託了那一莖牡丹分外精神。廳內陳列奢華，正中央一
張雕花楠木青漆嵌玉大圓桌，四周十二張鑲銀象牙扶手紅木椅，杯碗
勺箸，入眼生輝。    這裡便是醉仙樓的玫瑰廳，桌上平鋪著一份菜
單(menu)。
LONG );
    else
...
void create() {
    ...
    set("long", @LONG
進門繞過一道淡紅綢屏風，迎面牆上掛著一副『牡丹爭豔』圖，牡丹是中國
國花，素以富貴著稱。圖側對聯: 幽徑天姿呈獨秀，古園國色冠群芳。襯託了那
...
int do_broadcast(string arg) {
    ...
    if (type == "marry") {
        ...
        mesg = me->name() + "現在在醉仙樓牡丹廳舉行生辰壽宴了  !!!\n";
    } else if (type == "player") {
        mesg = me->name() + "現在在醉仙樓牡丹廳宴請各路英雄豪傑  !!!\n";
    }
