...
create_food(string nmstr, string* idlist, string longstr)
{
        ...
        if (longstr)
        set("long", @LONG
進門繞過一道淡綠綢屏風，迎面牆上掛著一副『芙蓉出水』圖，
工筆彩繪，狀極清雅，氣質卓絕。廳內陳列奢華，正中央一張雕花楠
木青漆嵌玉大圓桌，四周十二張鑲銀象牙扶手紅木椅，杯碗勺箸，入
眼生輝。    這裡便是醉仙樓的玫瑰廳，桌上平鋪著一份菜單(menu)。
LONG );
        else
              set("long", GRN+"一盤熱氣騰騰,香噴噴的"+nmstr+"。\n"+NOR, obj);
...
void create()
{
        set("short", HIY"芙蓉宴廳"NOR);
        set("long", @LONG
進門繞過一道淡綠綢屏風，迎面牆上掛著一副『芙蓉出水』圖，工筆彩繪，
狀極清雅，氣質卓絕。廳內陳列奢華，正中央一張雕花楠木青漆嵌玉大圓桌，四
周十二張鑲銀象牙扶手紅木椅，杯碗勺箸，入眼生輝。
    這裡便是醉仙樓的玫瑰廳，桌上平鋪著一份菜單(menu)。
LONG );
        set("exits", ([ /* sizeof() == 1 */
            "south" : "/d/city/zxlpath",
        ]));
        set("no_sleep_room",1);
        set("item_desc", ([
                "menu": @TEXT
...
void init()
{
        ...
        if (query("order_owner")) {
                me = this_player();
                set("food",query("food",  me)/2, me);
                set("water",query("water",  me)/2, me);
        }
}
...
int do_broadcast(string arg)
{
        ...
        me = this_player();
        if( query("order_owner") != query("id", me) )
                return 0;
        type = query("order_type");
        if (type == "marry") {
                mesg = me->name() + "現在在醉仙樓芙蓉廳舉行訂婚酒宴了  !!!\n";
        } else if (type == "birthday") {
                mesg = me->name() + "現在在醉仙樓芙蓉廳舉行生辰壽宴了  !!!\n";
        } else if (type == "player") {
                mesg = me->name() + "現在在醉仙樓芙蓉廳宴請各路英雄豪傑  !!!\n";
       }
