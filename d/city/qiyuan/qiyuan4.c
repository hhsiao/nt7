// 棋苑
// 標準棋房
// 注意 棋房 一開始下棋後就不能允許有人走動
// 不然正在下的棋房上的變數就要重新被初始了
// 所以最好在下棋後禁止人物走動
// 詳細參看 /d/city/qiyuan/qiyuan1.c  - Luky
...
桌，上面划著一張圍棋盤。桌角擺放著棋子。另外還有兩張石凳，顏
...
void  check_players()
{
        ...
        if(pl["white"])  {
                ...
                if(!objectp(wp)  ||  !living(wp)  ||  !present(wp,rm))  {
                        map_delete(pl,"white");
                        if( living(wp))delete_temp("weiqi_seat", wp);
                }
        }
