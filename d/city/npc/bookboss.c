...
void create()
{
        ...
        set("chat_msg",(
        {
                "老闆手裡拿著一把雞毛撣子，輕輕撣去牆上，書架上的塵土。\n",
                "老闆笑著問道：“買書？還是看看字畫兒？”\n",
                "老闆輕聲嘟喃著:識貨者分文不取，不識者千金難求。\n",
...
int ask_me()
{
        if ((int)this_player()->query_skill("literate",1)<125){
                say("辛友清白了白眼，對" + this_player()->name() + "說道：怎麼看你也不象個讀書人。\n");
                return 1;
        }else{
                say("辛友清看了" + this_player()->name() + "一眼,神神秘秘的說道：\n"
                    "請問您是誰的後人(answer)?\n");
                set_temp("pending/answer", 1, this_player());
                return 1;
        }
}
int ask_gong()
...
int do_answer(string arg)
{
        ...
        if( !query_temp("pending/answer", ob))return 0;
        delete_temp("pending/answer", ob);
        if (arg == "先祖司馬遷" && query("book_count"))
        {
                addn("book_count", -1);
                book = new("/clone/book/lbook9");
                if ( book->violate_unique())
                {
                        destruct(book);
                        write("辛友清衝著" + ob->name() + "說道：可惜你晚來一步啊。\n");
                        return 1;
                }
                say("辛友清對" + ob->name() + "說道：今兒終於遇到恩人的後代拉。\n"
                    "我把先生的遺書交還給您拉。\n");
                book->move(ob);
                message_vision("$N交給$n一本書。\n", me, ob);
                command("rumor "+query("name", ob)+"拿到史記真篇啦。\n");
                return 1;
        }
