...
int accept_object(object who, object ob) {
    ...
    {
        if(query_temp("shagu_silver", who)){
            tell_object(who, "傻姑接過圖解，倒著看了半天，不明所以，大怒之下，將圖解撕得稀爛！\n");
            destruct(ob);
            return notify_fail("");
        }
        else {
            if (query_temp("silver_give")) {
                tell_object(who, "傻姑接過圖解，倒著看了半天，不明所以，只好又還了給你。\n");
                return notify_fail("");
            }
            else {
                tell_object(who, "傻姑接過圖解，倒著看了半天，不明所以，只好又還了給你。\n");
