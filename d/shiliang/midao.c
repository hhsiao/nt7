...
來。洞壁(wall)上滿是滑溜溜的青苔。很奇怪的是這裡有根長
繩(string)，繩端繫著一個大石頭(stone)。
LONG
    );
    set("coor/x", 1600);
    set("coor/y", -1980);
...
int do_reach(string arg) {
    ...
    if(arg=="stone")
    {
        message_vision("$N試著去抓石頭，但總是差一點夠著，要能跳高一點就好了。\n", me);
    }
    if(arg=="string")
    {
        message_vision("$N試著去抓長繩，但總是差一點夠著，要能跳高一點就好了。\n", me);
    }
    return 1;
}
int do_climb(string arg) {
    ...
    if(arg=="string" )
    {
        if(query_temp("hasjump", me) )
        {
            ...
            message_vision("$N順著長繩往上爬，終於爬出了山洞。\n", me);
            message_vision("$N揉了揉眼睛，彷彿還不適應這裡的光線，好象有個人在面前。\n", me);
