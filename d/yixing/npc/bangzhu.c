...
#include "/d/yixing/doc/info_biao.h"
#include "/d/yixing/doc/info_guest.h"
#include "/d/yixing/doc/info_destine.h"
...
void create() {
    ...
    if(temp = sizeof(obj) ) {
        for(i = 0; i < temp; i++)
            names-=({ query("party/party_name", obj[i]) });
    }
...
int accept_kill(object me) {
    ...
    say(name() + "說道：你想謀害本幫主，當真是吃了熊心豹子膽了！！\n");
...
void die() {
    ...
    if(objectp(killer = query_temp("last_damage_from")) ) {
        ob = new(BANGYIN);
        set("long", query("title") + query("name", ob) + "。\n", ob);
        ob->move(environment());
...
#include "/d/yixing/npc/bangzhu_duty.h"
#include "/d/yixing/doc/set_bang.h"
