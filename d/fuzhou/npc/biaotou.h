...
int ask_jiaobiao()
{
        ...
        {
                message_vision("$N指著$n說道：“我不是和他一路的，我不認識他。”\n",biaotou, me);
                return 1;
        }
        tell_object(me,HIW+query("name", ob)+"笑道：恭喜你成功將鏢車送到目的地，完成了這項任務！\n\n"NOR);

        message("channel:chat",HIM"【謠言】聽說"+query("guardby", cart)+"成功將鏢車送到目的地了！\n"NOR,users());
        ...
        tell_object(me,sprintf(HIR+query("name", ob)+"對"+query("name", me)+"稱許道：“你被獎勵了！得到%s點經驗和%s點潛能和%s點江湖閱歷。鏢局發給你四十兩黃金做你的薪金和補償你的押金。”\n"NOR,chinese_number(give_exp),chinese_number(give_pot),chinese_number(give_score))+NOR);
