...
int perform(object me, object target){
    string msg;
    int extra, bonus, lvl, str;
    object weapon;
    weapon = me->query_temp("weapon");
    lvl = me->query_skill("tianya-blade", 1);
    if (me->query("class")!= "bandit")
        return notify_fail("你的天涯明月刀已經失去了功效。\n");
    if (lvl < 50)
        return notify_fail("你的［天涯明月刀］還不夠純熟！\n");
    ...
    if (lvl <100) {
        ...
        刀既出，天上的玉鉤也失去了光華，地上的萬物都屏吸注視著$N"HIC"手中一彎明月。刀光如月，月光如刀。"NOR;
        ...
        刀既出，天上的玉鉤也失去了光華，地上的萬物都屏吸注視著$N"WHT"手中一彎明月。刀光如月，月光如刀。"NOR;
        COMBAT_D->do_attack(me, target, me->query_temp("weapon"), TYPE_REGULAR, msg);
        msg = HIY"『天涯明月刀』" NOR;
        COMBAT_D->do_attack(me, target, me->query_temp("weapon"), TYPE_REGULAR, msg);
        combat_message_vision(HIC"刀光隱，明月現。\n"NOR, target);
        me->add_temp("apply/attack",-extra);
        me->add_temp("apply/damage",-extra - bonus);
        me->start_busy(2);
    }
