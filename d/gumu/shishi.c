...
容易，室頂又矮，伸長手臂，幾可碰到。在石室角落裡有一隻不大的布
袋(budai) ，繩索綁著袋口，蠕蠕而動，不知道是什麼東西。
LONG );
        set("exits", ([
                "north" : __DIR__"mudao16",
        ]));
        set("item_desc", ([
                "budai": "一隻舊布袋，裡面還蠕蠕而動著。\n"
        ]));
        set("no_clean_up", 0);
        set("coor/x", -3200);
        set("coor/y", -60);
...
int do_open(string arg)
{
        ...
        if( query("opened", this_object()) )
                return notify_fail("布袋已被打開了。裡面啥也沒有了。\n");
        message_vision("$N俯身在石室角落裡提起一隻布袋，解開縛在袋口的繩索，倒轉袋子一抖，飛出來三隻麻雀。\n",me) ;
        set("opened", 1, this_object());
...
int do_zhua(string arg)
{
        ...
        {
                me->improve_skill("yunv-shenfa",random(query("dex", me)));
