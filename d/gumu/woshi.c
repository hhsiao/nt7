...
void create()
{
        ...
        set("long", @LONG
這裡是小龍女居住的臥室，房中空空洞洞。右首是一塊長條青石作
床 (bed)，床上鋪了張草蓆，一幅白布當作薄被，此外除了一根人高的
繩索橫過室中，室左立著一個粗糙木櫥(chu)。
LONG        );
        set("item_desc", ([
                "bed" : HIC"一張由普通青石鋪成的床，床角落處，有一塊突起的石板，似可扳動。\n"NOR, 
                "chu" : HIY"一座普通之極的櫥子。\n"NOR, 
        ]));
        set("exits", ([
...
int do_tang(string arg)
{
        ...
        if ( arg == "bed")
        {
                message_vision(HIC"$N俯附身躺在青石製成的石床上面。\n"NOR,me);
                set_temp("marks/床", 1, me);
                return 1;
        }
...
{  
        ...
        if( !query_temp("marks/床", me) )
                return notify_fail("你沒躺床上，恐怕用不上力吧！\n");        
        if (me->is_busy() || me->is_fighting())
