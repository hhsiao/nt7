...
int accept_fight(object ob)
{
        ...
        if( query("damaged", me) && query("id", ob) != "mamagic" && query("id", ob) != "lily" && query("id", ob) != "elf" )
                return notify_fail("這個木人已經被打壞了！\n");        

        if( random(query("fight_times", me)) >= 10 && query("id", ob) != "mamagic" && query("id", ob) != "lily" && query("id", ob) != "elf"){
                set("damaged", 1, me);
                return notify_fail("這個木人已經被打壞了！\n");        
        }

        if( query("last_fighter", me) == query("id", ob) && query("id", ob) != "mamagic" && query("id", ob) != "lily" && query("id", ob) != "elf" )
