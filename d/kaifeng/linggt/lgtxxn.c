...
#define BONUS_INTERVAL 8        //每間隔幾層獲得一次獎勵機會
#define TIME_KICK 90            //每層過關的基礎戰鬥時間
#define CAN_USEKNIFE 3          //限制每輪闖關可以使用飛刀靈符的次數
#define CAN_USECRYSTAL 3        //限制每輪闖關可以使用塔靈水晶的次數，每塊-15秒，使用後的所有層都有效
#define BONUS_EXPADD 2200
...
void kickdown(object me) {
    ...
    if (!query_temp("lgt/knock2", me)) {
        i = (query("no_magic")) ? (TIME_KICK - query_temp("lgt/tlcrystal", me)*15) : 24;
        ...
        if (j > i ) {
            ...
            write(YEL + "你聽到無數佛陀在耳邊梵唱，莫大的鼓舞使你精神振奮...\n" + NOR);
...
int do_use(string arg) {
    ...
    if(arg == "knife" || arg == "fly knife" || arg == "fei dao") {
        ...
        tell_object(me, CYN + "你拿出一塊" + query("name", obb) + NOR + CYN + "捏成粉碎，夢幻泡影般的紫青迷霧揮發，時間流逝變快了...\n" + NOR);
        return 1;
    }
