...
void create() {
    set_name(YEL "傀儡衛" NOR, ({ "kuilei wei", "kui lei", "kuilei", "wei" }));
    ...
    set("death_msg", NOR "\n$N" + NOR + (random(2)? "身形" : "腳步") + (random(2)? "一錯" : "一晃") + (random(2)? "消失" : "隱匿") + "了。\n" NOR);
    set_heart_beat(2);
    setup();
}
...
void heart_beat() {
    object armor, owner, me = this_object();
    ...
    if (query("is_shadow") ) {
        owner = query("shadow_own");
        if (!owner || !objectp(owner) || !owner->is_fighting() ) {
            unconcious();
            return;
        }
        armor = query("armor_own");
        if (!armor || !objectp(armor) || !query("equipped", armor) ) {
            unconcious();
            return;
        }
    }
...
void sp_attack() {
    ...
    smo = query("smash_ob");
    if (enemy && smo && !query_temp("sma_time")) {
        if (smo->can_smash(this_object(), enemy, 10)) {
            set_temp("sma_time", 1);
            return;
        }
    }
...
varargs void die()
{
    ...
    if (!query("is_shadow", this_object())) {
        if (killer && objectp(killer) && environment(killer)==environment(this_object())) GIFT_D->delay_bonus(killer,
            ([ "exp": BONUS_EXP, "pot": BONUS_EXP / 3, "mar": BONUS_EXP / 6, "prompt": "你在洞天擊退" + name() + "之後"]));
        }
