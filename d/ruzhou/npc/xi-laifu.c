...
void create() {
    ...
    set("inquiry", ([
        ...
        "here": "喜來福搖頭晃腦地說：“我們醉仙樓可是方圓幾十裡的名樓，辦喜宴還得要來我們這裡。”\n",
        "宴" : (: ask_party :),
        "喜宴" : (: ask_party :),
        "席" : (: ask_party :),
        "酒席" : (: ask_party :),
    ...
    setup();
    set_max_encumbrance(1000000000000);
...
int ask_party() {
    ...
    if(!(wid = query("couple/couple_id", me)) )
    {
        message_vision("$N對$n說：新娘子都沒來，這喜宴怎麼開？\n", ob, me);
        return 1;
    }
    if(!objectp(wife = present(wid, environment(me))) )
    ...
    if(query("short", environment(ob)) != "醉仙樓大堂" )
    {
...
void start_party(object ob, object me) {
    command("chat* "+name() + "一拱手道：“良宵一刻值千金，酒不醉人人自醉，今日醉仙樓榮幸地為"+query("name1")+
        "和"+query("name2") + "\n大開喜宴，恭迎各位前來捧場！\n\n");
...
int do_start() {
    ...
    while (i--)
    {
        if(obj = new("/d/ruzhou/npc/obj/xijiu"))
            obj->move(list[i]);
    }
...
int do_serve(string arg) {
    ...
    if(food = new("/d/ruzhou/npc/obj/jiuyan"))
...
void finish_party(object me, object ob) {
    ...
    while (i--)
    {
        ob2 = new("/d/ruzhou/npc/obj/xitang");
...
int do_steal(string arg) {
    ...
    tell_object(me, "你還沒搞清楚是什麼事，就被一群捕快拖著出了醉仙樓......\n");
...
int kick(object me) {
    ...
    tell_room("/d/city/beidajie2", "突然一個人影從西邊的醉仙樓二樓飛出來，“蓬”地重重摔在地上，細看卻是一個差點摔死的人。\n");
    set_temp("kick", 1, me);
    me->move("/d/city/beidajie2");  //移到樓下的大街房間裡即可
