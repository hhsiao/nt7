...
int do_chat()
{
   ...
   message_vision(HIW "江知府臉色一沉，驚堂木一拍喝道：“大堂之上，竟敢喧譁！來人呀！”\n"
...
int zou(object me)
{
   message_vision(HIW"一群衙役衝上前來，不由分說，把$N掀翻在地，一陣大板隨即跟上……\n"NOR,me);
...
//        if( !ob->restore() )
        ...
        if( file_size(DATA_DIR + "login/" + arg[0..0] + "/" + arg + ".o")<0 )
   {
       ...
       "道：“送一份給紅娘莊！”然後又對$N說：“你走吧，你已經是單身了！”\n",me);
                ...
                sprintf( "經本府明查：由於%s的配偶已經去世，他們的婚姻關係自然解除！\n", me->name()));
   ...
   set_temp("unmarry", 1, obj);
        message_vision(HIY "江耀亭看了看點點頭說道：“$N和$n，既然你們都同意離婚，那本官也就給你們辦了！”\n"
           "說完，在姻緣簿上重重地劃去了兩個名字。\n" NOR, obj, me);
        CHANNEL_D->do_channel(this_object(), "mudnews",
        sprintf( "經本府宣判：從今天起，%s和%s的婚姻關係解除！\n", me->name(), obj->name()));
