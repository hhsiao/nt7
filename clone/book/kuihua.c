...
int do_du(string arg) {
    ...
    else if (me->query_skill("bixie-jian", 1) > 74) neili_lost = 10;
    else neili_lost = 5;
    ...
    if(query("combat_exp", me)<bxlevel*bxlevel*bxlevel / 10 )
    {
    ...
    if(!me->query_skill("bixie-jian", 1) ) {
        ...
        write("                   《葵花寶典》閱讀指南\n"
            "\n    恭喜你開始研習武林第一功夫－－－－辟邪劍法！\n"
            "注意事項：\n"
            "一．研習辟邪劍法必先自宮，如你不想讓別人發現這個天大的秘密，你必須\n"
            "    找一無人所在之處閱讀，且當有別的玩家在場時不要使用辟邪劍法戰鬥，\n"
            "    因為當你閱讀或使用辟邪劍法時會出現一些陰柔的舉動，使別人產生懷\n"
...
int do_join(string arg) {
    ...
    if(!query_temp("pending/join", this_player()) )
        return 0;

    set("rank_info/respect", "公公", this_player());
    set("rank_info/rude", "閹賊", this_player());
    write("恭喜你成為朝廷最重要的機關東廠的一員！\n");
    delete_temp("pending/join", this_player());
    return 1;
}
