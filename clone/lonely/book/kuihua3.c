...
void return_to_liang();
void start_borrowing() {
    remove_call_out("return_to_liang");
    call_out("return_to_liang", 14400 + random(7200));
}
...
int do_yanjiu(string arg) {
    ...
    if (pfm != "gui" && pfm != "duo" && pfm != "pi" && pfm != "po" && pfm!= "wu" && pfm!="you")
    {
    ...
    if (pfm == "wu")
    {
        if(query("can_perform/pixie-jian/wu", me) )
        {
            write("你已經學會了這招，再研究也沒用。\n");
            return 1;
        }
        ...
        } else
        {
            ...
            if (me->can_improve_skill("pixie-jian"))
                me->improve_skill("pixie-jian", 1500000);
            me->improve_skill("martial-cognize", 1500000);
            set("can_perform/pixie-jian/wu", 1, me);
        }
        me->receive_damage("jing", 80);
        addn("learned_points", 10, me);
        me->start_busy(2);
    ...
    if (pfm == "you")
    {
        if(query("can_perform/pixie-jian/you", me) )
        {
            write("你已經學會了這招，再研究也沒用。\n");
            return 1;
        }
        ...
        } else
        {
            ...
            if (me->can_improve_skill("pixie-jian"))
                me->improve_skill("pixie-jian", 1500000);
            me->improve_skill("martial-cognize", 1500000);
            set("can_perform/pixie-jian/you", 1, me);
        }
        me->receive_damage("jing", 80);
        addn("learned_points", 10, me);
        me->start_busy(2);
...
void return_to_liang() {
    ...
    if(base_name(me)=="/d/fuzhou/liang" )
    {
        remove_call_out("return_to_liang");
        call_out("return_to_liang", 14400 + random(7200));
        return;
    ...
    if (playerp(me))
    {
        ...
        message_vision("忽然一陣狂風捲著沙石向你捲來，你連忙拿手遮起眼睛，狂風逐漸平靜下來"
            "你覺得身上好像少了點什麼？又說不上來。\n", me);
    } else
    {
        message("vision", "忽然一陣狂風捲著沙石捲來，把地上一張紅色的袈裟捲上天空"
            "消失了。\n", me);
    }

    this_object()->move("/d/fuzhou/liang");
}
