...
void create()
{
        set_name(HIW"白色藥粉"NOR, ({ "white yao","yao"}) );
        /*if( clonep() )
...
void init()
{
        add_action("do_wpour", "wpour");
        add_action("do_tu", "tu");
}

int do_wpour(string arg)
{
        string what;
...
//        object *inv;
        ...
        if( !arg || sscanf(arg, "%s in %s", who, what)!=2 || victim == me)
                return notify_fail("命令格式: wpour <人物> in <物品>。\n");
...
{
        this_player()->apply_condition("xiezi_poison",
                (int)this_player()->query_condition("xiezi_poison")
