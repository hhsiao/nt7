...
void init() {
    add_action("do_bpour", "bpour");
    add_action("do_tu", "tu");
}

int do_bpour(string arg) {
    string what;
    ...
    if(!arg || sscanf(arg, "%s in %s", who, what)!=2 ||victim == me)
        return notify_fail("命令格式: bpour <人物> in <物品>。\n");
    ...
    {
        int myskill = (me->query_skill("five-poison", 1))*10 + query("neili", me);
        ...
        message("vision", "你與" + victim->name() + "擦身而過，偷偷將《" + name() + "》灑進他的" + ob->name() + "裡面。\n", this_player());
        add_amount(-1);
        me->start_busy(2);
        if(random(myskill) < skill / 2 )
            message("vision", "你看到"+me->name() + "鬼鬼祟祟地在"+victim->name() + "身上的一"+query("unit", ob) + ob->name() + "裡灑入了一些"+name() + "！\n", environment(me), ({me, victim}));
        return 1;
    }
...
int drink_drug(object ob) {
    this_player()->apply_condition("chanchu_poison", this_player()->query_condition("chanchu_poison") + query("liquid/slumber_effect", ob));
    return 0;
}
