...
void create() {
    set_name(HIG"綠色藥粉"NOR, ({ "green yao", "yao" }) );
...
int do_gpour(string arg) {
    ...
    if(!arg || sscanf(arg, "%s in %s", who, what)!=2 || victim == me)
        return notify_fail("命令格式: gpour <人物> in <物品>。\n");
...
int drink_drug(object ob) {
    this_player()->apply_condition("zhizhu_poison", this_player()->query_condition("zhizhu_poison") + query("liquid/slumber_effect", ob));
    return 0;
}
