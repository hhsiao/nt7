...
int do_addin(string arg)
{
    ...
    if( query("id", me) == P1Data["Id"]
        ...
        if (!Playing)
            message_vision("$N大喊: [欠腳啊～～] 不停的哭哭啼啼.....。\n",me);
        else
            message_vision("$N對著大家說: 今天自摸"+(random(12)+1)+"次就好。\n",me);
        return 1;
    }
    if (Playing) {
        if (random(2))
            ...
            message_vision("$N喊著: [我也要玩啦～～] 不停的哭哭啼啼.....。\n",me);
        return 1;
    }
    if (BetAmount>0) {
...
int do_dump(string str,object me)
{
    ...
    if (check==0)
        return notify_fail("沒有["+str+"]這種代碼。\n");
    check=do_check_Mj(who["Mj"],str);
    if (check==0)
        return notify_fail("你沒"+do_check_num(str,0)+"這張牌。\n");
...
　　每胡就有底台３，在加上所得的台就是總台，要是自摸就＊３，相當於三一的算法。
...
int do_bet(string arg)
{
    ...
    if(!arg){
            if(BetAmount<=0) command("say 目前並未設定賭注，純練功夫。");
