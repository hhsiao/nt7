...
#include "ansi2.h"
inherit NPC;
#define MJ_DATA "/data/game/mj_data.o"
#define END 286
#define START 0
...
int do_restart(string str)
{
        ...
        message_vision("$N說道：牌局重新設定請用"+HIR+" addin "+NOR+"加入牌局。\n",this_object());
        NowPlayerId[0]="";
        NowPlayerId[1]="";
        NowPlayerId[2]="";
        NowPlayerId[3]="";
...
//        for (i=0;i<sizeof(X);i++) printf("%s=%d ",X[i],MjE_Data[X[i]]);
        ...
        i%=Play;
        i++;
        message_vision("["+str+chinese_number(i)+"]牌局開始:莊家"+HIY+NowPlayerId[0]+HIR+countstr+NOR+"擲出骰子"+
...
int do_setmj(string str)
{
        ...
        if (!str){
          ...
          if (who["AutoGon"]!="") numstr+="遇到"+HIY+show_mj(who["AutoGon"],2)+NOR+"會自動"+HIY+"[  牌]"+NOR+"。\n";
        ...
        if (str=="tin"){
           ...
           if (numstr=="none"){
              who["AutoGon"]="";
              return notify_fail("你取消自動  牌設定。\n");
           }
           check=do_check_Mj(who["Mj"],numstr);
           if (check==0) return notify_fail("你沒"+do_check_num(numstr,0)+"這張牌。\n");
           if (check < 3) return notify_fail("你手中要有叄張"+do_check_num(numstr,0)+"才能設定  此牌。\n");
           check=do_check_Mj(who["AutoGon"],numstr);
           if (check){
              who["AutoGon"]=do_delete_Mj(who["AutoGon"],numstr,1);
              return notify_fail("你"+HIR+"刪掉了"+NOR+"遇到"+do_check_num(numstr,0)+"會自動  牌的設定。\n");
           }
           who["AutoGon"]+=numstr;
           Auto_Gon=1;
           write("你設定遇到"+do_check_num(numstr,0)+"時會自動  牌。\n");
           return 1;
        }
        else if (str=="pon"){
           if (numstr=="none"){
