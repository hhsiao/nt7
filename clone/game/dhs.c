...
void create()
{
        set_name("大話色臺", ({ "da hua se desk","se","desk","dhs"}) );
        set_weight(1);
    if( clonep() )
        ...
        {
                set("unit", "張");
        set("long", "這是一張大話色臺，玩家可以在上面玩大話色，使用方法請使用(helpse)命令。\n");
        set("no_get", 1);
        set("value", 1);
        set("material", "wood");
        }
...
string for_other_msg(object who)
{
        ...
        if(has_guess&&last_player)
        {
                ...
                if(ob)
                {
                        m=sprintf("%s\n%s(%s)猜色子數為：%d個%d\n",m,query("name", ob),query("id", ob),cur_amount,cur_point);
                        if(ob!=who)
                                m=m+"如果你不相信，可以拆穿大話("HIY"dahua"+query("id", ob)+NOR")\n";
                }
        }
        return m;
}
...
string for_guess_msg()
{
        ...
        m = m+"你的猜測是("HIY"guess 色子數目 色子點數"NOR")？\n";
        if(has_guess&&last_player)
                m = m+"注意：" HIC "色子數目或者色子點數必須比前一位玩家大。\n"NOR;
        return m;
}
...
int is_visitor(object who,string which)
{
        ...
        if( !visitor[which] || member_array(query("id", who),visitor[which]) == -1 )
                return 0;
        return 1;
}
...
void toss(object ob)
{
        ...
        if(!ob)
                return;
        if( s=query("id", player[ob)] )
        {
                for(i=0;i<sizeof(s);i++)
                        s[i] = random(6)+1;
        }
        query("id", player[ob)]=sort_array(s,1);
}
...
int do_toss(string arg)
{
        ...
        if(has_guess)
                return notify_fail("已經開始猜色子了。\n");
        if( query("id", player_toss[me)] >= MAX_TOSS )
                return notify_fail(sprintf("你已經搖過%d次了。\n",MAX_TOSS));

        toss(me);
        query("id", player_toss[me)]++;
        msg(me,0,"$N拿起色子在手中搖了兩搖。\n");
        view_se(me,0);

        if( idx=query("id", visitor[me)] )
        {
                for(i=0;i<sizeof(idx);i++)
                {
                        v = get_player(idx[i]);
...
int do_start(string arg)
{
        ...
        {
                if(i<1||i>MAX_SE)
                        return notify_fail(sprintf("start [色子數目]  *注意* 色子數目必須>1並且<=%d\n",MAX_SE));
        }
        start_game(i);
        return 1;
}
...
int do_join(string arg)
{
        ...
        query("id", player[me)]=allocate(se_num);
        toss(me);
        msg(me,0,"$N加入遊戲了。\n");
        return 1;
}
...
int do_guess(string arg)
{
        ...
        if(!arg||sscanf(arg,"%d %d",n,p)!=2)
                return notify_fail("guess 色子數目 色子點數\n");

        if(n<sizeof(player)||p>6||p<=0)
                return notify_fail(sprintf("色子數目必須>=%d，色子點數必須>0並且<=6\n",sizeof(player)));
        ...
        if(!ok)
                return notify_fail("色子數目或者色子點數必須比前一位玩家大。\n");

        last_player=query("id", me);
        cur_amount = n;
        cur_point = p;
        ...
        if(history)
                history=history+({sprintf("%-20s：%d個%d",query("name", me)+"("+query("id", me)+")",n,p)});
        else
                history=({sprintf("%-20s：%d個%d",query("name", me)+"("+query("id", me)+")",n,p)});
...
int show_all_se(object who)
{
        ...
        m=sprintf("%s%s猜有"HIC"%d"NOR"個%d，",m,query("name", ob),cur_amount,cur_point);
        m = sprintf("%s現在有"HIC"%d"NOR"個%d\n\n",m,c,cur_point);
        msg(0,0,m);

        if( query("id", who) == cur_player )
                pt = 1;
        else
                pt = 2;
        ...
        if(c<cur_amount)
        {
                add_counter(query("id", who),pt);
                add_counter(query("id", ob),-pt);
...
int do_finish(string arg)
{
        ...
        if(sizeof(player_deal)==sizeof(player))
        {
                ...
                who_lost = 0;
                if( !cur_player)cur_player=query("id", me);
...
int do_visitor(string arg)
{
        ...
        if(is_playing(me))
        {
                ...
                if(del)
                {
                        if( !is_visitor(ob,query("id", me)) )
                                return notify_fail("不是你的觀戰者。\n");

                        msg(me,ob,"$N不再允許$n觀看$N的戰況。\n");
                        visitor[query("id", me)]=visitor[query("id", me)]-({query("id", ob)});
                }
                else
                {
                        if( is_visitor(ob,query("id", me)) )
                        ...
                        if( !visitor[query("id", me)] )
                                visitor[query("id", me)]=({query("id", ob)});
                        else
                                query("id", visitor[me)]+=query("id", ob);
                }
        }
        else
        {
                ...
                tell_object(ob,"接受使用[visit"+query("id", me)+"]。\n");
        }
        return 1;
}
