...
void create()
{
        set_name("海戰棋", ({ "sea board","board","seaf"}) );
        set_weight(1);
    if( clonep() )
        ...
        {
                set("unit", "副");
        set("long", "這是一副海戰棋，使用方法請使用(helpboard)命令。\n");
        set("value", 1);
        set("material", "wood");
        }
    reset_game(1);
...
int do_join(string arg)
{
        ...
        query("id", player[me)]=sizeof(player)+1;
        msg(me,0,"$N加入遊戲了。\n");
...
int do_unplace(string arg)
{
        ...
        for(i=0;i<SHIP_NUM;i++)
        {
                if(sh[i][1]==x&&sh[i][2]==y)
                {
                        ...
                        sh[i][2] = 0;
                        tb[x-1][y-1][0] = 0;
                        display(player_id(query("id", this_player())));
                        break;
                }
        }
...
int do_rplace(string arg)
{
        ...
        sh=query("id", get_ship(this_player()));
        tb=query("id", get_table(this_player()));
        for(i=0;i<SHIP_NUM;i++)
        {
                if(!sh||!tb)
                        return notify_fail("access date fail\n");
        ...
        display(query("id", player_id(this_player())));
        return 1;
}
