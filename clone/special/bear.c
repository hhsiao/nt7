...
是愛的蠱惑 讓我又興起貪求的念頭 
有多愛我 夠不夠久 會不會走 
藏在柔順背後 你忠於自我 情愛裡遊走 從不曾見你低頭 
我卻常犯錯 像一個太忙太累太傻的陀螺 
轉個不休 只放不收 停不了手 

太想愛你是我壓抑不了的念頭 想要全面佔領你的喜怒哀愁 
你已征服了我 卻還不屬於我 叫我如何不去猜測你在想什麼 
太想愛你是我壓抑不了的折磨 能否請你不要 不要選擇閃躲 
...
敏銳的心是否會更脆弱 
真正有心的人我才說 
You know I'm asking for the moon 
Would you make my dream come true? 
如果可能我不想再孤獨 
每一顆星每一個夜都有月光停駐 
我要等待祝福 
Well,baby am I asking for the moon 
Oh,baby am I asking for the moon
MUSIC,
@MUSIC
悠長的假期
在自己什麼都做不好的時候 
...
作白日夢的人 上不了最前線 
跟勇氣通上電 讓脆弱站到一邊 
搖醒你的心 跟希望 SAY HI , SAY HI 
COME ON , COME ON 給我感覺 
給我 給我真的感覺 (OH YEH) 
陽光之下把心敞開 
愛就愛 敢愛敢做的人超級精彩 HE YA , HE YA 
COME ON , COME ON 給我感覺 
給我 給我真的感覺 (OH YEH) 
大口呼吸自由自在 
愛就愛 誰能阻止我們開懷 
...
void create()
{
        set_name("玩具熊", ({"xiong","toy"}));
        set_weight(80);
        set("long", "一隻姿態逗人的玩具熊,特大號的。在熊的肚子上有一個按鈕(press)。");
        set("unit", "只");
        set("value", 0);
        play = -1;
}
...
void init()
{
        add_action("do_music", "press");
        add_action("do_new", "newsong");
}
...
int play_music()
{
        ...
        if (sen < sizeof(vision))
        {
                h = sen==0? "＊":"～";
                message_vision(HIB + h + vision[sen] + "\n" NOR, this_object());
                sen++;
                call_out("play_music",3);
        } else
        {
                message_vision(HIB "％\n" NOR, this_object());
                play = -1;
...
int do_music(string arg)
{
        ...
        if (play != -1)
        {
                ...
                play = -1;
                remove_call_out("play_music");
                return 1;
        }
        
        play = random(sizeof(music));
        sen = 0;
        return play_music();
}
