...
void create() {
    ...
    set_name(HIY "炎黃Ⅱ中秋月餅" NOR, ({ "yh2 yuebing2", "yh2", "yuebing2" }) );
    set_weight(1);
    msg = HIW "各位江湖豪傑：\n"
...
int do_effect(object me) {
    ...
    if(! objectp(present("yh2 yuebing2", me)))return 0;

    tell_object(me, HIY "你哈哈一笑，將整個炎黃Ⅱ中秋月餅一起吞了。\n" NOR, me);

    tihui = 300000 + random(200001);
    ...
    if (me->query("combat_exp") > 5000)
        tell_object(me, HIM "你獲得了五十萬點經驗和一百萬點潛能以及" + chinese_number(tihui) + "點實戰體會。\n" NOR, me);

    if (MEMBER_D->is_valib_member(me->query("id")))
        tell_object(me, HIM "你獲得了五萬炎黃幣(yhb)！\n" NOR, me);
