...
#include "harbor.h"
    ...
    if (room->query("boat") && stringp(room->query("opposite"))) {
        rivers["cross_river"] = room->query("opposite");
    }

    // connect all the islands
    if (room->query("navigate") && stringp(room->query("opposite"))) {
        rivers["out_sea"] = room->query("opposite");
    }
...
void mapit(int max_node) {
    int current_total;
    object cr;
    object mapdb;

    if (!queue) return;     // make sure we have a queue to use
    if (map_ready) return;  // prevent ready-map from being overwritten
