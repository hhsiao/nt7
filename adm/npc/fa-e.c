...
#define SHIZHE DATA_DIR + "npc/fa-e"
...
*/
        ...
        set("no_get", 1);
        set("auto_perform", 1);
        set("startroom", "/d/taishan/zhengqi");
        check_clone();
}
...
int accept_fight(object ob)
{
        ...
        if( query("id", this_player()) == name1){
                command("say 盟主大人，在下罰惡使者怎麼是您的對手？！\n");
                return 0;
        }

        if( !(shangshan = find_living("shangshan shizhe")) )
                if( !(shangshan = find_object(SHANGSHAN)) )
                        shangshan = load_object(SHANGSHAN);
        shangshan->restore();
        name2=query("winner", shangshan);
        if( query("id", this_player()) == name2){
                command("say 你是賞善使者，我是罰惡使者，我們打什麼架？！\n");
                return 0;
        }
...
void lost()
{
        ...
        command("say 果然厲害，恭喜你成為當今罰惡使者！\n");
        command("chat 哈哈哈，到底是長江後浪推前浪，一代新人換舊人！\n");
        command("chat恭喜"+query("name", ob)+"被推舉為當今罰惡使者！\n");
        set("fighting", 1, me);
        remove_call_out("do_copy");
        call_out("do_copy", 1, me, ob);
...
int do_copy(object me, object ob)
{
        ...
        delete("party", ob);
        set("party/party_name", HIW+"第"+chinese_number(query("generation", me))+"代"+NOR, ob);
        set("party/rank", HIW+"罰惡使者"+NOR, ob);

        set("short", HIW+"第"+chinese_number(query("generation", me))+"代罰惡使者"+NOR+""+query("name", me)+"(Faeshizhe)", me);
        delete("title", me);
