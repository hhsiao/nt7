...
mixed ask_me2() {
    ...
    // || to_int(MEMBER_D->db_query_member(me, "endtime")) < 1888888888)
...
mixed ask_me5() {
    ...
    ret = MEMBER_D->db_query_member(me, ({"vip", "payvalue", "transfervalue"}));
    if (to_int(ret["vip"]) == 3)
        return "恭喜你，你已經是白金會員了！\n";

    set_temp("identify_member", t, me);

    value = to_int(ret["payvalue"]);
    trans = to_int(ret["transfervalue"]);
    value -= trans;
...
mixed ask_buchang() {
    ...
    {
        ret = MEMBER_D->db_query_member(id, ({"jointime", "endtime"}));
...
mixed ask_duanwu() {
    ...
    ret = MEMBER_D->db_query_member(id, ({"endtime"}));
    endtime = to_int(ret["endtime"]);
