...
int exec(object target, object from) {
    ...
    if(!is_root(previous_object()) ) {
        log_file("static/security",
            sprintf("%s  %s(%s) want to exec (%s)%s to (%s)%s.\n",
                ctime(time()),
...
void shutdown(int how) {
    ...
    db = find_object(DATABASE_D);
    if(objectp(db) ) destruct(db);
 ...
 * if( !is_root(previous_object()) ) {
 * log_file("static/security",
 * sprintf("%s  %s(%s) want to cp %s to %s.\n",
 * ctime(time()),
 * (string)base_name(previous_object()),
 * (this_player(1) ? geteuid(this_player(1)) : "???"),
 * (string)src,
 ...
 * if( !is_root(previous_object()) ) {
 * log_file("static/security",
 * sprintf("%s  %s(%s) want to rename %s to %s.\n",
 * ctime(time()),
 * (string)base_name(previous_object()),
 * (this_player(1) ? geteuid(this_player(1)) : "???"),
 * (string)src,
