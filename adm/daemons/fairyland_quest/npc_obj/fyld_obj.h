...
// 其他文件內使用這個方式隨機  set.
    ...
    set("msg", MAG"半空中忽地掉下一件物事，像是一件寶物！\n"NOR);
    if( iii !=3 )
    ...
    set("no_refresh", 1);  // 有此設置時，在 no_clean_up 的房間內，不會被刷新掉！
...
int do_zuhe(string arg)
{
    ...
    if( query("pivotal", ob) != "yes" )
        return notify_fail(query("name", ob)+"不是最重要的部分，不能由它開始組合。\n");
