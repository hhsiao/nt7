...
varargs int top_skill(object who,int l)
{
        ...
        {
                skname = sort_array( keys(skl), (: strcmp :) );
...
//返回頂經驗武功
       ...
       exper = (float)query("combat_exp", who);

        level = to_int(ceil( pow( exper*10.0, 0.333333)) );
...
/* skill prepare*/
        ...
        for ( i = 0; i < sizeof(inv); i++ )
                if( query("equipped", inv[i]) && stringp(weapon_type=query("skill_type", inv[i])) )
                {
        ...
        set("family_name",query("family/family_name",  thief_master), tob);
        if (flag == 1)  destruct(thief_master);
}
...
int set_perform(object who)
{
        ...
        mixed msg =({});
                ...
                set("chat_chance_combat", 60, who);
                set("chat_msg_combat", combat_actions, who);
        return 1;
}
...
int check_place(string file,string dir)
{
        ...
        if( query("no_fight", room))return 0;
        return 1;
}
...
//        return; //看一下是不是這裡造成lag
        ...
        } else
        {
                ...
                for (i = 0; i < 12; i++)
                {
                        ...
                        if( query("no_fight", dest))continue;
                        if( query("no_quest", dest))continue;
...
//                        tell_object(find_player("qingyun"),"nplace="+explode(file_name(find_object(dest)),"/")[<2]+"\n");
                        if( stringp(query("place", ob)) && explode(file_name(find_object(dest)),"/")[<2] != query("place", ob))continue;
                        if (stringp(query_reverse(dir))) break;
                }
        }
...
int c_here(object ob)
{
        ...
        if( query("no_fight", room))return notify_fail("不能打架。\n");
        seteuid(ROOT_UID);
        ...
        if (write_file(file_name(room)+".c",implode(newfile,"\n"),1))
        {
                ...
                tell_object(find_player("qingyun"),"寫入失敗。\n");
                return 1;
}
