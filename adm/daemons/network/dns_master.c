...
string start_message()
{
        return sprintf("||NAME:%s||VERSION:%s||MUDNAME:%s||DRIVER:%s||USERS:%d"
                       "||MUDLIB:%s||HOST:%s||PORT:%d||ENCODING:%s"
                       "||PORTUDP:%d||TIME:%s||ZONE:%s||TCP:%s||MUDTYPE:%s",
                       ...
                       my_port, ctime(time()), CONFIG_D->query_string("zone"),
                       TCP_SERVICE_LEVEL, MUD_TYPE);
}
...
void create()
{
        ...
        if (stringp(MUDLIST_DNS6))
                list_nodes += ({ MUDLIST_DNS6 });

        if (stringp(MUDLIST_DNS7))
                list_nodes += ({ MUDLIST_DNS7 });

        if (stringp(MUDLIST_DNS8))
                list_nodes += ({ MUDLIST_DNS8 });

        if (stringp(MUDLIST_DNS9))
                list_nodes += ({ MUDLIST_DNS9 });
