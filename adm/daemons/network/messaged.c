...
#if ENABLE_LOG
#define DEBUG_OUT(msg)          if (find_player("lonely")) tell_object(find_player("lonely"), (msg) + "\n")
#else
...
private void handle_msg(mapping info)
{
        ...
        if (objectp(me = connection[user]))
        {
                    ...
                    info[FUNCTION] != FUN_LOGOUT)
                {
                        ...
                        error_handle(error_msg("沒有你的登錄信息，請重新登錄。\n"));
                        return;
                }
        ...
        if (me)
                set_temp("last_error", last_error_msg, me);
}
...
private int user_logon(mapping info)
{
        ...
        if (! objectp(body) || ! body->restore())
        {
                ...
                return error_msg("無法讀取該用戶的信息，請和巫師聯繫。\n");
        }
        ...
        if (body->is_in_prison())
        {
                destruct(ob);
                destruct(body);
                return error_msg("你因為犯法太多，已經被關進監獄。\n");
        }
        ...
        {
                destruct(ob);
                return error_msg("暫時無法接入系統，請稍候再試。\n");
        }
        ...
        set_temp("link_ob", ob, me);
        set_temp("body_ob", me, ob);
        my = me->query_entire_temp_dbase();
...
void user_logout(mixed user, string msg)
{
        ...
        {
                // hide this object
                message("vision", HIY "一道金光閃過，" + user->name() +
                                  HIY "消失得無影無蹤。\n" NOR,
                        environment(user), ({ user }));
        }

        set_temp("logout_notice", msg, user);
        remove_user(user);
}
...
private void call_fun(mapping info)
{
        ...
        if (undefinedp(info[FUNCTION]))
        {
                ...
                if( stringp(inputed=query_temp("inputed", me)) &&
                    strlen(inputed) < (wizardp(me) ? MAX_WIZARD_INPUT_LEN
                                                   : MAX_PLAYER_INPUT_LEN))
                {
                        set_temp("inputed", inputed+info[MESSAGE], me);
                } else
                {
                        set_temp("inputed", info[MESSAGE], me);
                }
                is_send_ack = 1;
                return;
        }
...
private int fun_logout(mapping info)
{
        ...
        DEBUG_OUT(sprintf("Logout out: %d   %O = %d\n",
                          info[SEQUENCE],me,query_temp(SEQUENCE, me)));
        if( info[SEQUENCE] == query_temp(SEQUENCE, me) )
        {
...
private void reply_ack(object user)
{
        ...
        msg = sprintf("%s:%d", ACK, query_temp(SEQUENCE, user));
        if( query_temp("last_error", user) )
                msg+=sprintf(SEP"%s:%s",ERROR,query_temp("last_error", user));

        DEBUG_OUT(sprintf("Send ack(%s) to %s(%s)", msg,
                  query("id", user),query_temp(ADDRESS, user)));
        send_udp(query_temp(ADDRESS, user),msg);
}
...
private int queue_msg(object msgto, mapping send_info, mixed user, mixed finish)
{
        if (! msgto->is_chatter())
        {
                ...
                else
                {
                        ...
                        case FUN_ACKTELL:
                                tell_object(msgto, sprintf(HIG "你告訴%s" HIG
                                                           "(%s)：%s\n" NOR,
                                                           send_info[NAME],
                                                           send_info[USER],
                                                           send_info[MESSAGE]));
        ...
        {
                notice_user(user, FUN_NOTICE, "系統正繁忙，無法代你發送該消息。\n");
                return 0;
        }
...
private void save_data(object user)
{
        ...
        // save the dbase of the user
        login_ob=query_temp("link_ob", user);
        body = 0;
        for (;;)
        {
                if (! login_ob)
                {
                        ...
                        set("id", geteuid(user), login_ob);
                        if (! login_ob->restore())
                                break;
                }

                // record login object data
                set("last_on", time(), login_ob);
                set("last_from",query_temp(ADDRESS,  user), login_ob);
                login_ob->save();
...
string reject_tell(object user1, object user2)
{
        ...
        no_tell=query("env/no_tell", user2);
        can_tell=query("env/can_tell", user2);

        to_name = user2->name(1) + "(" + query("id", user2) + ")";
        if (! wiz_level(user1) && (no_tell == "all" || no_tell == "ALL" ||
            is_sub(query("id", user1),no_tell)) )
        {
...
void remove_user(object user)
{
        ...
        if (connection[query("id", user)] == user)
        {
                // Am I in connection now ?
                map_delete(connection,query("id", user));
                save_data(user);
        }
        ...
        if( query_temp(ADDRESS, user) &&
            stringp(msg=query_temp("logout_notice", user)) )
        {
                send_udp(query_temp(ADDRESS, user),
                         sprintf("%s:%s" SEP "%s:%s",
                                 FUNCTION, FUN_LOGOUT,
                                 MESSAGE, msg));
        }

        if (objectp(query_temp("link_ob", user)))
                // Is the link object existed now ?
                destruct(query_temp("link_ob", user));
...
int send_msg_to(mixed user, string msgto, string msg)
{
        ...
        if (! objectp(from = user) &&
            ! objectp(from = find_user(user)))
                return error_msg("你沒有正常登錄，無法進行交談。\n");
        ...
        if (! to->is_chatter())
                set_temp("reply",query("id",  from), to);
        ...
        // remote tell
        queue_msg(to, ([ USER : query("id", from),
                         NAME : from->name(1),
                         FUNCTION : FUN_TELL,
                         MESSAGE : msg ]),
...
{
        if (! to || ! from) return;
        queue_msg(from, ([ USER : query("id", to),
                           NAME : to->name(1),
...
void create()
{
        ...
        foreach (ob in obs)
        {
                if( query_temp("disconnected", ob) )
                {
                        // this object should be removed
                        destruct(ob);
                        continue;
                }
                connection[query("id", ob)] = ob;
        }
