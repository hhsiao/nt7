...
//   &&   base_name(previous_object()) != TELL_CMD) return;
   ...
   msg=replace_string(msg,"$P",gender_pronoun(query("gender", me)));
   ...
   if (info["NAME"] && info["PORTUDP"]) {
     ...
     if(objectp(target)) {
       if(target->is_character()&&
          query("env/invisibility", target)<1){
          msg=info["MSG"];
          msg=replace_string(msg, "$R", RANK_D->query_respect(target));
          msg=replace_string(msg, "$r", RANK_D->query_rude(target));
          msg=replace_string(msg,"$n",target->name()+"("+query("id", target)+"@"+Mud_name()+")");
          msg=replace_string(msg,"$p",gender_pronoun(query("gender", target)));
                  }
                }
