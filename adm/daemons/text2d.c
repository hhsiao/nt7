// Writed by Hcifml
// 點陣系統，呼叫的時候需要兩個參數一個是需要
// 圖符化的字符串，一個是點陣大小，另外四個參
// 數分別表示填充圖符，背景填充圖符，顏色，背
// 景顏色，為可選參數。
...
varargs string display(string str, int size, string fill, string bg, string fcolor, string bgcolor)
{
        ...
        if (! fill || sizeof(fill) != sizeof(bg))
                fill = DEFAULT_FILL;

        if (! fcolor)
                fcolor = DEFAULT_FCOLOR;

        if (! bg || sizeof(fill) != sizeof(bg))
                bg = DEFAULT_BG;
        ...
        for (int k = 0; k < sizeof(str); k++)
        {
                ...
                if (! sizeof(char))
                        error("TEXT to PIC can't read bytes from character lib\n");
