...
varargs int do_channel(object me, string channel, string msg, mixed raw)
{
    ...
    if(query("chblk_tt", me) > time() ) {
        me->start_busy(10);
        return notify_fail("你的聊天頻道給關閉了！等" + chinese_number(abs(query("chblk_tt", me) - time()) / 60 + 1) + "分鐘後自動開通。\n");
    }
    ...
    if(!undefinedp(raw) || emote )
        ...
        send_msg = channel_title + time + color + idname + (idname != "" ? "：" : "") + color;
    ...
    if(userp(me) && channel == "chat" && random(100) < 20 )
    {
        ...
        if ((time() - krok_award) > 14400)
        {
            ...
            if (krokob1 && userp(krokob1)) {
                tell_object(krokob1, HIY "\a你在聊天大會上勇奪" HBRED HIY "麥霸" NOR HIY "榮譽，獲獎鋼鏰和二冊萬象書。\n" NOR);
            ...
            if (krokob2 && userp(krokob2)) {
                tell_object(krokob2, HIY "\a你在聊天大會上勇奪" HBRED HIY "話癆" NOR HIY "榮譽，獲獎鋼鏰和十張金票。\n" NOR);
                item = new("/u/redl/npc/obj/penny");
                item->move(krokob2, 1);
