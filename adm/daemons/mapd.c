...
mapping map_trans = ([
    ...
    "jiaxing": ([ "name": "嘉興", "room": "/d/jiaxing/majiu", ]),
    "jingzhou": ([ "name": "荊州", "room": "/d/jingzhou/majiu", ]),
    "nanyang": ([ "name": "南陽", "room": "/d/nanyang/majiu", ]),
    "changan": ([ "name": "長安", "room": "/d/changan/majiu", ]),
    "hanzhong": ([ "name": "漢中鎮", "room": "/d/hanzhong/majiu", ]),
...
public string show_map(object env) {
    int i, j, jj = 0, k, kk = 0;
    string file, map, name, dest;
    mapping exit1, exit2, exit3;
    object room2, room3, room4;
...
int arrived_dest(object me, mixed follower, object running, object horse, string trans_path) {
    ...
    if (objectp(horse))
        tell_object(me, "\n" + horse->name() + HIW "到了目的地，長嘶一聲，猛然停下，頓時塵土四散……\n" NOR);
    //if (objectp(me))
    //{
    me->move(trans_path);
    ...
    if (objectp(horse))
        message("vision", "\n" + this_object()->name() + HIW "到了目的地，長嘶一聲，猛然"
