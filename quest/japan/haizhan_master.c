...
void check_hp() {
    ...
    if (random((int)me->query_skill("swimming", 1) ) < 80 ) {
        ...
        tell_object(me, CYN"你奮力向岸邊游去，雖然根本就看不見岸在哪裡……\n"NOR);
        me->unconcious();
    }
...
int do_liaowang(string arg ) {
    ...
    if (!arg || sscanf(arg, "%d %d", x, y ) != 2 ) return notify_fail("請輸入正確座標！x -> 豎著數第幾橫  y -> 橫著數第幾個\n");

    if (x < 1 || x > 10 || y < 1 || y > 16 ) return notify_fail("請輸入正確座標！x -> 豎著數第幾橫  y -> 橫著數第幾個\n");
