...
void test(object here)
{
	...
	list = filter_array(users(), (: $1->query("family/famly_name")=="峨嵋派":));
	...
	while( i-- ) {
		...
		if ( list[i]->query("combat_exp") < 10000) continue;
		cnt++;
	...
	CHANNEL_D->do_channel(ob, "party", "元兵攻山，" + s + "等待援兵，殺退敵人！\n");
...
int do_huzhuo()
{
	...
	if ( !me->query("quest/jiuyin2/emei") || !me->query("quest/jiuyin2/emeijump"))
	...
	if(book){
		...
		message_vision("滅絕師太從斷刀斷劍中取出了幾張帛卷，看了幾眼，想了想又收了回去。\n", me);
		tell_object(me,"看來有人捷足先登了。\n");
	}
	...
	log_file("quest/jiuyin",sprintf("%-18s在娥眉九陰部分失敗%s次後成功從滅絕師太處得到娥眉九陰。\n",
		me->name(1)+"("+capitalize(getuid(me))+")",chinese_number(me->query("quest/jiuyin2/pass"))), me);
