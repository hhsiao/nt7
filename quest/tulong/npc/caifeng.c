...
int do_ding(string arg)
{
        ...
        return notify_fail("你現在只能擁有"+chinese_number(query("age", me)/11)+
"件自造物品。\n");
        if( query("created_item", me) >= 3 )
        return notify_fail("你已經擁有太多自造物品了。\n");
        ...
        return notify_fail("你身上並沒有"+material+"。\n");
        if( !query("for_create_cloth", ob2) )
        return notify_fail(ob2->name()+"並不可以用來做衣飾。\n");
        ...
        newfile = replace_string( newfile, "訂作的", cname);
        ...
        me->start_busy(1);
        write("你將身上的"+ob2->name()+"交給"+name()+"，打造成一"+query("unit", ob)+
        ob->name()+"\n");
        destruct(ob2);
        seteuid(getuid());
        return 1;
