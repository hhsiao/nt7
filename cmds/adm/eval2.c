...
mixed eval(string euid)
{
    seteuid(euid);
    {
EndText + doith(replace_string(arg, "\n", "")) + ";\n\t}\n}\n";
    ...
    if (!err)
    {
        ...
        rm(filename);
        if(ret = find_object(filename) ) destruct(ret);
        set("channel_id", sprintf("%s(%s)", me->name(), capitalize(query("id", me))));
        CHANNEL_D->do_channel(this_object(), "sys", sprintf("eval %s", arg));
    }
    else
        write("執行失敗！\n");
    return 1;
