...
int main(object me, string str) {
    ...
    foreach (ob in ob_list)
    {
        ...
        delta = dbase["int"] - query("gift/int/succeed", ob) - query("gift/water", ob )
        +dbase["con"] - query("gift/con/succeed", ob) - query("gift/lighting", ob )
        +dbase["str"] - query("gift/str/succeed", ob) - query("gift/sun", ob )
        +dbase["dex"] - query("gift/dex/succeed", ob) - query("gift/feng", ob )
        - 84;
...
int sort_user(object ob1, object ob2) {
    return query("mud_age", ob2) - query("mud_age", ob1);
}
