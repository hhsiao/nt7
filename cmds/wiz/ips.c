...
protected int check_multilogin() {
    ...
    for (i = 0;i<s1;i++) {
        ...
        sql = sprintf("SELECT place,address FROM %s WHERE ipstart <= \"%s\" AND"
            " ipend >= \"%s\" ORDER BY ipend LIMIT 1", IP_TABLE, ip_number, ip_number);
        if(sizeof(ret = DATABASE_D->db_fetch_row(sql)) )
        {
            place = ret[0];
            address = ret[1];
            printf("%-50s%d%8s", (ip_addr[i] + "("+place + ","+address + ")"), count[i], " ");
        } else
        printf("%-50s%d%8s", ip_addr[i], count[i], " ");
