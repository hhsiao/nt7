...
void init()
{
        ...
        if( interactive(ob) && !query("no_fight", environment(ob)) && 
                ...
                !(query_temp("zuji_target", me) != t_name)) )
        {
                set_temp("apply/attack", random(20), me);
                set_temp("apply/defense", random(20), me);
//                me->set_temp("apply/damage",  random(20));
                set_temp("apply/dodge", 50+random(50), me);
...
void dest2()
{
        ...
        write(query("name", me)+"道：反正信已到手，就饒你一命！\n",me);
        write(query("name", me)+"匆匆離開。\n",me);
