...
int perform(object me, object target)
{
        ...
        if( target->query_skill_mapped("sword") != "quanzhen-jian" )
                return notify_fail("你想與其雙劍合璧的對象所使用的並非全真劍法！\n");

        if( !objectp(weapon=query_temp("weapon", target) )
                 || query("skill_type", weapon) != "sword" )
                return notify_fail("你想與其雙劍合璧的對象並沒有用劍！\n");

        if( BUFF_D->check_buff(me, "yunv_hebi") )
                return notify_fail("你正與別人進行雙劍合璧。\n");

        if( BUFF_D->check_buff(target, "quanzhen_hebi") )
...
void check_fight(object me, object target, object victim)
{
                ...
                query("qi", victim) <= 50 && me->is_fighting(victim) && target->is_fighting(victim) )
        {
                ...
                BUFF_D->debuff(target, "quanzhen_hebi", 1);
        ...
        else if( me->query_skill_mapped("sword") != "yunv-jian")
        {
                BUFF_D->debuff(me, "yunv_hebi", 1);
                BUFF_D->debuff(target, "quanzhen_hebi", 1);
        ...
        else if( target->query_skill_mapped("sword") != "quanzhen-jian")
        {
                BUFF_D->debuff(me, "yunv_hebi", 1);
                BUFF_D->debuff(target, "quanzhen_hebi", 1);
