...
int valid_learn(object me)
{
        ...
        if( query("max_neili", me)<400 )
                return notify_fail("你的內力修為不足，無法學習八十一路潑水杖。\n");

        if ((int)me->query_skill("force") < 60)
                return notify_fail("你的內功火候太淺，無法學習八十一路潑水杖。\n");

        if ((int)me->query_skill("staff", 1) < 20)
                return notify_fail("你的基本杖法太淺，無法學習八十一路潑水杖。\n");

        if ((int)me->query_skill("staff", 1) < (int)me->query_skill("poshui-zhang", 1))
                return notify_fail("你的基本杖法水平有限，無法領會更高深的八十一路潑水杖。\n");
...
{
        int i, level;
        level = (int)me->query_skill("poshui-zhang", 1);
...
int practice_skill(object me)
{
        ...
        if( query("qi", me)<80 )
                return notify_fail("你的體力不夠練八十一路潑水杖。\n");

        if( query("neili", me)<80 )
                return notify_fail("你的內力不夠練八十一路潑水杖。\n");
...
{
        return __DIR__"poshui-zhang/" + action;
}