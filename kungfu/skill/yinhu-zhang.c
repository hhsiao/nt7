...
mapping *action = ({
([      "action":"$N右手托住杖端，一招「天上地下」，左掌居中一擊，令其憑慣性倒向$n的肩頭",
        "force" : 65,
        "attack": 12,
        "dodge" : -10,
        "parry" : 5,
...
([      "action":"橫持$w，一招「海中求月」，杖端化出無數個圓圈，凝滯沉重，把$n纏在其中",
        "force" : 280,
        "attack": 67,
        "dodge" : 5,
        "parry" : 22,
...
([      "action":"$N全身滾倒，$w盤地橫飛，突出一招「巨浪滔天」，杖影把$n裹了起來",
        "force" : 320,
        "attack": 89,
        "dodge" : -5,
        "parry" : 28,
...
int valid_learn(object me)
{
        ...
        if ((int)query("max_neili", me) < 200)
                return notify_fail("你的內力修為不足，無法學習銀瑚杖法。\n");

        if ((int)me->query_skill("force", 1) < 140)
                return notify_fail("你的內功火候太淺，無法學習銀瑚杖法。\n");

        if ((int)me->query_skill("staff", 1) < 60)
                return notify_fail("你的基本杖法太淺，無法學習銀瑚杖法。\n");

        if ((int)me->query_skill("staff", 1) < (int)me->query_skill("yinhu-zhang", 1))
                return notify_fail("你的基本杖法水平有限，無法領會更高深的銀瑚杖法。\n");
...
int practice_skill(object me)
{
        ...
        if ((int)query("qi", me) < 80)
                return notify_fail("你的體力不夠練銀瑚杖法。\n");

        if ((int)query("neili", me) < 80)
                return notify_fail("你的內力不夠練銀瑚杖法。\n");
