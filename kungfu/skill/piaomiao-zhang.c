...
int valid_learn(object me) {
    if(query_temp("weapon", me) || query_temp("secondary_weapon", me) )
        return notify_fail("練縹緲掌法必須空手。\n");

    if ((int)me->query_skill("force") < 30)
        return notify_fail("你的內功火候不夠，無法學縹緲掌法。\n");

    if(query("max_neili", me)<100 )
        return notify_fail("你的內力太弱，無法練縹緲掌法。\n");
    ...
    if ((int)me->query_skill("strike", 1) < (int)me->query_skill("piaomiao-zhang", 1))
        return notify_fail("你的基本掌法水平有限，無法領會更高深的縹緲掌法。\n");
...
int practice_skill(object me) {
    ...
    if(query("neili", me)<50 )
        return notify_fail("你的內力不夠練縹緲掌法。\n");
