...
int update_condition(object me, int duration) {
    ...
    if(query_temp("guosc_mis_flag", me) )
    {
        if(query_temp("guosc_mis_where", me) != query("short", environment(me)) )
        {
            message_vision(HIY"一個宋兵跑過來對$N說道：叫你守城，你卻到處閒逛，我去報告郭大俠！\n"NOR, me);
        }
        else set_temp("guosc_mis_flag", 2, me);
        if(query_temp("guosc_mis_flag", me) == 2 && query_temp("guosc_mis_where", me) == query("short", environment(me)) )
        {
            switch(random(8) )
            {
            case 0:
                ...
                break;
            case 1:
                message_vision(HIY"$N指揮一隊宋兵吆喝著將無數巨石從城牆上推下來，城下的"HIR"蒙古兵"HIY"一時不備，死傷不少。\n"NOR, me);
                break;
            case 2:
                message_vision(HIC"但聽一聲梆子響，從箭垛後面鑽出無數士兵，箭如飛蝗，城外的"HIR"蒙古兵"HIC"紛紛中箭倒下。\n"NOR, me);
                break;
            case 3:
                message_vision(HIR"$N大叫：放火、放火。城上扔下無數燃著的火把，慘叫聲中，蒙古的士兵死傷甚眾。\n"NOR, me);
                break;
            case 4:
            ...
            case 5:
                ...
                break;
            case 6:
                message_vision(HIR"城中宋兵大叫：著火啦、著火啦。城中的蒙古奸細趁守軍不備四處放火，一片救火聲中，城中的軍備損失慘重！\n"NOR, me);
                me->receive_wound("jing", 50 + random(50));
                me->receive_damage("qi", 300 + random(100));
                break;
            case 7:
