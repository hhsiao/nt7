...
void create () {
    set_name("圍棋棋盤", ({"weiqi board", "board"}));
    set_weight(90000000);
    set ("long", @LONG
        這是一張圍棋棋盤，如何使用請參照 help weiqi
LONG);
    set("unit", "張");
    set("value", 0);
    set("no_get", 1);
...
void check_players() {
    ...
    if(pl["black"]) {
        ...
        if(!objectp(bp) || !living(bp) || !present(bp, rm)) {
            map_delete(pl, "black");
            if(living(bp)) delete_temp("weiqi_seat", bp);
        }
    }
    if(pl["white"]) {
        wp = pl["white"];
        if(!objectp(wp) || !living(wp) || !present(wp, rm)) {
            map_delete(pl, "white");
            if(living(wp)) delete_temp("weiqi_seat", bp);
        }
    }
...
int do_leave(string arg) {
    ...
    s = (string)(query_temp("weiqi_seat", me));
...
int do_play(string arg) {
    ...
    if(wf) {
        ...
        tell_room(rm, me->name(1) + "的圍棋技能得到了提高！\n");
        me->improve_skill("art", random(10) + 30);
