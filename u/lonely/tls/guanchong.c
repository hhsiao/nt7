...
string look_left(object me)
{
	return YEL"這張圖譜是一個卷軸舒開，帛面年深日久，已成焦黃之色，圖上開頭寫著： \n"+
		  ...
		  "著內力的運走徑道，旁邊注著一行小字“關衝劍運氣法門”。\n" NOR;
}

string look_right(object me)
{
	return YEL"這張圖譜是一個卷軸舒開，帛面年深日久，已成焦黃之色，圖上都是縱橫交叉\n"+
		  "的直線、圓圈和弧形，但見紅線黑線，縱橫交錯，頭緒紛繁之極，這圖便是關\n"+
		  "衝劍的劍法，這路劍法以拙滯古樸取勝，一招一式看似笨拙無比，其實包含極\n"+
		  "大的後著。\n" NOR;
}
...
int do_study(string arg)
{
	...
	if(arg=="left" ) {
		...
		if (neili < (j+1)*3000+1500 && !me->query("liumai-shenjian/guanchong"))
			return notify_fail("你竭盡全力的驅動體內的一陽指真氣，試圖由關衝穴射出無形劍氣，卻無半點勁氣發出。\n");
		...
		if ( lvl >= 30*(j+1) || me->query("liumai-shenjian/guanchong",1)){
			write("你內力運轉自如，凝聚真氣，無名指一出，嗤嗤聲響，一條無形劍氣自“關衝”穴中激射而出。\n");
			...
			write("你照著圖譜研習關衝劍法的運氣法門，漸漸的悟出其中的精要所在。\n");
			if (!me->query("liumai-shenjian/guanchong"))
				me->improve_skill("liumai-shenjian", me->query("int"));
		}
		return 1;
