...
mixed accept_ask(object who, string topic) {
    ...
    if (! this_object()->is_waiter()
        && (who->query("ultra_quest/next_npc") == filter_color(this_object()->name())
        || who->query("ultra_quest/npc") == filter_color(this_object()->name())))
...
int accept_object(object who, object ob) {
    ...
    if (! this_object()->is_waiter()
        && (who->query("ultra_quest/next_npc") == filter_color(this_object()->name())
        || who->query("ultra_quest/quest/obj") == filter_color(this_object()->name())))
...
void dialog_end(object me, int flag) {
    ...
    sTianshu = this_object()->query("skybook");

    if (! sizeof(sTianshu))return;

    if (! objectp(ob = find_object("/clone/skybook14/" + sTianshu + "/" + sTianshu)))
        ob = load_object("/clone/skybook14/" + sTianshu + "/" + sTianshu);

    ob->dialog_end(me, flag);
    return;
}
