        ...
        if( !mapp(act) )
        {
                ...
                if( !mapp(act) )
                {
                        CHANNEL_D->do_channel( this_object(), "sys",
                                        ...
                                        query("actions", me)));
                        tell_object(me, HIR "你的兵器出現問題，請更換現有的兵器！\n" NOR);
...
varargs int do_attack(object me, object victim, object weapon, int attack_type, int flag)
{
        ...
        // 新手村不可以pk
        // 殺萬年烏龜處不能PK
        //if( environment(me)->short() == "九頭蛇領地" && userp(me) && userp(victim) )
        ...
        */
        ...
        { // 減少判斷，節約資源
                ...
                dp = query_temp("buff_combat/dp", victim) * 4 / 5; // 連招持續弱化對手防禦
        }
        else
        {
                ...
                //if( victime->is_freeze() ) dp /= 10; // 冷凍狀態下，躲閃能力降低90/100
        }
        ...
        if( (sizeof(query_temp("guarded", victim)) || array_flag == 1) && do_protect(me, weapon, ap, victim) )
        {
                ...
                if( random(ap + dp) < dp ) {
                        ...
                        { // 減少判斷，節約資源
                                pp = query_temp("buff_combat/pp", victim) * 4 / 5; // 連招持續弱化對手防禦
                        }
                        else
                        {
