...
void create() {
    set("short", "哀牢山");
    set("long",
        "這裡是哀牢山東麓，峰頂站著幾位修道者，山腳下是滾滾流淌的\n"
        "元江。兩隻金蛛噴射出一股股粗大的蛛絲，死死捆住了元江裡一隻巨\n"
        "大的金船，正賣力地往上拉。\n"
    );
...
int c_ghost(int blv, int si) {
    ...
    for(int i = 0; i < (23 - blv * 2); i++ ) {
        ...
        if (tsi > 999) tsi = 999;
        //      DEBUG_CHANNEL("廣成金船投放水鬼等級：" + (string)tsi);
...
int yuanci_attack(object me) {
    ...
    message_vision(NOR + HIY "廣成金船"HIW"底部嗡地一響，一股漩渦之力吸住了$N往下拉，$N悶哼一聲精氣倒錯，受了嚴重內傷。\n\n" NOR, me);
    if (!random(5) && !me->is_weak()) {
        message_vision(NOR CYN "$N" BLU "被磁力死死壓住，渾身痠軟虛弱。\n", me);
        me->set_weak(28, 0);
    }
    if (!random(7) && !me->query_condition("no_perform")) {
        message_vision(NOR CYN "$N" BLU "奇經八脈被磁力封住，無法施展絕招。\n", me);
    ...
    if (my["eff_qi"] < 2000000 * iyca || my["eff_jing"] < 1000000 * iyca) {
        set_temp("die_reason", NOR + "被元江水眼的地肺真磁力壓死了" + NOR, me);
        my["qi"] = my["eff_qi"] = -1;
...
int deploy() {
    ...
    if (!blv) { //開新船
        if (query("cusr_name")) {
            ...
            CHANNEL_D->channel_broadcast("chat", "鄭顛仙(zheng dianxian)：已經借到金蛛打撈廣成金船，有意者速來飛星法陣哀牢山。");
            blv = 10;
    ...
    if (blv < 8 && uc < 1) {
        CHANNEL_D->channel_broadcast("news", HIG "聽說去廣成金船裡尋寶的玩家已經全軍覆沒。");
        delete("blv");
        return 1;
    }
    ...
    foreach(usr in allusrs) {
        tell_object(usr, HIW "\n船外傳來鄭顛仙的一聲大喝，蛛絲似乎力有不逮，整個大船緩緩往下沉去。\n\n" NOR);
        usr->start_busy(2);
    ...
    if (allusrs && uc) {
        ...
        booki = 2400 - (time() / 86400 / 7);    //手動得出的，給先進行這個任務2月內的玩家優惠獎勵，逐周下降，直到1倍為止
        if (booki < 1) booki = 1;
        booki = booki * uc * 10;
    ...
    if (blv>0) {
        if (blv < 8) {
            ...
            if (cint<6) {
                if (random(8<<(cint - 1))==0) {
                    ...
                    tell_object(usr, HIC "你吞下"HIY"廣成金丹"HIC"先天悟性永久提高１點。\n" NOR);
                    CHANNEL_D->channel_broadcast("news", HIG "聽說" + HIY + query("name", usr) + HIG + "服用" HIY "廣成金丹"HIG"先天悟性永久提高１點。\n");
                } else {
                    if (random(2))
                        tell_object(usr, HIC "你吞下的這顆"HIY"廣成金丹"HIC"似乎沒有產生效果。\n" NOR);
                    else {
                        ...
                        tell_object(usr, HIC "你的額外內力上限增加了"HIY + chinese_number(addnneili) + HIC"點。\n" NOR);
        ...
        if (!random(3)) {
            door = get_object("/u/redl/teleport/yuanjiang/npc/obj/door");
            tell_room(environment(door), NOR + door->name() + NOR + "打了個噴嚏，居然長出了兩隻腳，撒腿跑了。\n" NOR);
            door->move(get_object(__DIR__"yuanjiang/chuan" + (string)random(50)));
            set("exits/out"  , __DIR__"yuanjiang/chuan" + (string)random(50) , environment(door));
        }
    ...
    //DEBUG_CHANNEL("廣成金船沉沒，玩家存活數：" + (string)uc);
    foreach(usr in allusrs) {
        if (!msg) msg = NOR + HIC + query("name", usr) + NOR;
        else msg += HIG "、" + NOR + HIC + query("name", usr) + NOR;
        tell_object(usr, HIW "鄭顛仙大叫一聲“不好~！”你一個激靈，運起輕功跳回岸上，只見巨船轟隆轟隆地往江底沉去。\n" NOR);
        //usr->move(query("exits/out", this_object()));
        usr->move(this_object());
        addn("weiwang", -100000, usr);
        tell_object(usr, NOR "由於元江奪寶失敗，你被扣除十萬點江湖威望。\n"+ NOR);
    }
    msg = sort_msg(HIG "聽說金蛛力竭，廣成金船重新沉沒，" + msg + HIG "僥倖逃出生天。" + NOR);
...
int valid_leave(object me, string dir) {
    ...
    if (dir=="wiz" && wiz_level(me) < 3) {
        tell_object(me, NOR + "你不能走那條通道，具體諮詢(ask zheng all)。\n" + NOR);
        return -1;
    }
