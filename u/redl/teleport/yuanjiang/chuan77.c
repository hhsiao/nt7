...
LONG );
        set("objects", ([ 
                "/clone/fam/max/zaohua" : random(8)+16,
                "/d/dongtian/obj/leaf" : random(3)+2,
                "/u/redl/npc/obj/ntb" : random(2)+2,
                "/clone/goods/tiandilu" : 1,
                "/u/redl/teleport/npc/obj/qkfz" : random(2)+2,
                        __DIR__"npc/liu" : (!random(40) ? 1 : 0),
        ])); 
        set("stop_run", 1);
                set("no_protect", 1);
                set("no_rideto", 1);
...
void init()
{
        ...
        add_action("do_esc", "out"); 
                ...
                for(int i = 0; i < maxc; i++ ) {
                        ...
                        set("guarder_level", random(50) + 800, npc);
                        npc->do_copyskill();
