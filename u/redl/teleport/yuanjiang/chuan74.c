...
LONG );
        set("objects", ([ 
                "/clone/fam/max/zaohua" : random(2)+4,
                "/d/dongtian/obj/leaf" : random(3)+1,
                "/d/dongtian/obj/yuanjing" : random(2),
                        __DIR__"npc/liu" : (!random(70) ? 1 : 0),
        ])); 
        set("stop_run", 1);
                set("no_protect", 1);
                set("no_rideto", 1);
...
void init()
{
        ...
        add_action("do_esc", "out"); 
                ...
                for(int i = 0; i < maxc; i++ ) {
                        ...
                        set("guarder_level", random(50) + 500, npc);
                        npc->do_copyskill();
