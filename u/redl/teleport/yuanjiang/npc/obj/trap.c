...
{ 
              set_name(NOR HIB "陷阱"NOR, ({"trap A"}) ); 
                ...
                set("long", NOR "這是金船裡的陷阱，叫人防不勝防，頭疼不已。\n" NOR);
                set("unit", "個");
                set("value", 1);
                set("no_uget", 1);
                set("no_drop", 1);
...
void do_trap1(object me)
{
        ...
        message_vision( NOR + HIB ".陷阱." HIK "嘣地一跳，一陣牛毛細針射在$N的身上流血不止。\n" NOR, me);
...
void do_trap2(object me)
{
        ...
        if (!environment(me) || environment(me)!=environment(this_object()))    return;
        message_vision( NOR + HIB ".陷阱." HIK "嘣地一跳，一陣刺鼻氣味燻得$N掩鼻欲嘔。\n" NOR, me);
...
void do_trap3(object me)
{
        int dmg = 150000 + random(50000);
        if (!environment(me) || environment(me)!=environment(this_object()))    return;
        message_vision( NOR + HIB ".陷阱." HIK "嘣地一跳，地板上伸出一排鐵籤刺穿了$N的腳掌。\n" NOR, me);
...
void do_trap4(object me)
{
        int dmg = 150000 + random(50000);
        if (!environment(me) || environment(me)!=environment(this_object()))    return;
        message_vision( NOR + HIB ".陷阱." HIK "嘣地一跳，牆上一根鐵管噴出一陣玄霜，把$N凍了起來。\n" NOR, me);
        me->set_freeze(45, 0);
        if( !me->is_busy() ) me->start_busy(random(4)+1);
