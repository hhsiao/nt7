...
int summ_ghost()
{
        ...
        addn("ghostn", 1);
        if (query("ghostn") > 4) return 1;
        npc = new(__DIR__"ghost");
        set("guarder_level", query("guarder_level")/2 + random(query("guarder_level")/2), npc);
        npc->do_copyskill();
        delete("chat_chance", npc);
        npc->move(environment(this_object()));
        npc->command("follow liu gen");
        return 1;
}
...
LONG);
        ...
        set_heart_beat(2);
        setup();
        set_skill("huisheng-jue", 3000);
        set_skill("taoism", 3000);
        set_skill("qingmu-zhenqi", 1000000);
        map_skill("taoism", "qingmu-zhenqi");
                carry_object("/kungfu/class/quanzhen/obj/greenrobe")->wear();
                reset_action();
}
