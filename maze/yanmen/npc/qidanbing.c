...
void create()
{
        ...
        set_name("契丹軍官",({ "menggu junguan","junguan" }) );
        set("gender", "男性" );
        set("long","這是一名契丹將領，正帶著軍馬執行著戰鬥任務。\n");
...
{ 
  ...
  if ( jungong > 10 ) set("title",HIR"契丹萬夫長"NOR);
  else if ( jungong > 8 ) set("title",HIG"契丹軍那顏"NOR);
  else if ( jungong > 5 ) set("title",HIY"契丹千夫長"NOR);
  else set("title",HIC"契丹百夫長"NOR);
