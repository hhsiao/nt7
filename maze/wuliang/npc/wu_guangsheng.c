...
void create()
{
        ...
        carry_object("/maze/wuliang/npc/obj/changpao")->wear();
}
void die()
{
 ...
 if( objectp(me) && !present("yuguangbiao",environment(me)) && query(QUESTDIR1+"start", me) && !query(QUESTDIR1+"wuliangover", me) && !query_temp(QUESTDIR1+"jianying", me) || query_temp("marks/xiaoyao/gotowyz", me) )
 {
...
set_temp(QUESTDIR1+"jianying", 1, 	me);
    ...
    tell_object(me,HIC"\n你聽完"+ob->name()+"之後，還是手下不留情面，雙手一緊，一股內力蓬勃而發。\n"NOR);
	  tell_room(environment(me),HIC"\n吳光勝說完很多之後，然而不知為何，似乎氣血一震，好像失去了什麼似的，慢慢萎靡倒地。\n"NOR, ({ me }));
    message_vision(HIG"$n仰天長嘆，為什麼啊，為什麼，$N竟然如此狠心。\n"NOR, me, this_object());
    if( query_temp("marks/xiaoyao/gotowyz", me)){
    delete_temp("marks/xiaoyao/gotowyz", 	me);
        delete_temp(QUESTDIR1+"jianying", me);
       set("marks/xiaoyao", "gotofind", me);	
    	}
