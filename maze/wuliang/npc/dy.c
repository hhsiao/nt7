...
void greeting(object me)
{
	...
	if( !me->query_condition("killer") && query(QUESTDIR1+"start", me )
			 && !query_temp(QUESTDIR1+"see_duanyu1", me )
			 && !query_temp(QUESTDIR1+"see_duanyu2", me )
...
set_temp(QUESTDIR1+"see_duanyu2", 1, 		me);
		  me->start_busy(1);
		  command("crazy");
		  command("say");
    }
	...
	if(!me->query_condition("killer")
		 && query(QUESTDIR2+"start", me )
		 && query_temp(QUESTDIR2+"pushdown", me )
		 && !query(QUESTDIR2+"over", me) )
    {
...
int ask_home()
{
	...
	if(!me->query_condition("killer")
		 && query(QUESTDIR1+"start", me )
		 && query_temp(QUESTDIR1+"see_duanyu2", me )
		 && !query_temp(QUESTDIR1+"gohome", me )
		 && !query_temp(QUESTDIR1+"see_duanyu1", me )
		 && !query(QUESTDIR1+"wuliangover", me )
		 && query_temp(QUESTDIR1+"jianying", me )
		 && query("id", ob) == "duanyu" )
    {
		...
		me->start_busy(1);
		command("say");
set_temp(QUESTDIR1+"gohome", 1, 		me);
...
void outroom(object me)
{
  ...
  if(!me) return;
  if( !query(QUESTDIR1+"start", me))return ;
  if( !query_temp(QUESTDIR1+"gohome", me))return ;
  if( query_temp(QUESTDIR1+"see_duanyu1", me))return ;
  if( query(QUESTDIR1+"wuliangover", me))return ;

  if(environment(ob)!=environment(me)) 
  {
  	command("say 人呢？"+me->name()+"剛才不是還在嗎？");
delete("quest/天龍八部/凌波微步篇", 	me);
 ...
 set(QUESTDIR+"combat_exp",query("combat_exp",  me), 	me);	
	...
	if( query("per", me)>26 )
	  {
		...
		tell_room(environment(me),HIC"轉頭卻見"+query("name", me)+"也是一復神色怪怪的樣子，也似乎被什麼吸引住了似的。\n"NOR,({me}));
	  ...
	  {
		command("say"+query("name", me)+"，這位姐姐，你且仔細看看這玉像，真是太神色動人了。");
		  command("points");
		  tell_object(me,HIC"\n你側過身子看那玉像時，只見她眼光跟著轉將過來，便似活了一般。你大吃一驚，側頭向右，玉像的眼光似乎也對著你移動。不論\n"
			  "你站在那一邊，玉像的眼光始終向著你，眼光中的神色更是難以捉摸，似喜似愛，似是情意深摯，又似黯然神傷。\n"NOR);
		tell_room(environment(me),HIC"轉頭卻見"+query("name", me)+"也是一復神色怪怪的樣子，也似乎被什麼吸引住了似的。\n"NOR,({me}));
  ...
  {
 	command("say"+query("name", me)+"，也是同道中人，你且仔細看看這玉像，真是太神色動人了。");
	  command("points");
	  tell_object(me,HIC"\n你側過身子看那玉像時，只見她眼光跟著轉將過來，便似活了一般。你大吃一驚，側頭向右，玉像的眼光似乎也對著你移動。不論\n"
	  "你站在那一邊，玉像的眼光始終向著你，眼光中的神色更是難以捉摸，似喜似愛，似是情意深摯，又似黯然神傷。\n"NOR);
	tell_room(environment(me),HIC"轉頭卻見"+query("name", me)+"也是一復神色怪怪的樣子，也似乎被什麼吸引住了似的。\n"NOR,({me}));
	  tell_object(me,HIC"\n你又向玉像呆望良久，這才轉頭，見東壁上刮磨平整，刻著數十行字，都是“莊子”中的句子，大都出自“逍遙遊”、“養生主”、\n"
		  "“秋水”、“至樂”幾篇，筆法飄逸，似以極強腕力用利器刻成，每一筆都深入石壁幾近半寸。文末題著一行字雲：“逍遙子為秋\n"
		  "水妹書。洞中無日月，人間至樂也。”！\n"NOR);
      if( query("gender", me) == "無性")tell_object(me,HIR"\n你暗想：“我要是真能做個女人，就是死一千遍，一萬遍，也如身登極樂，歡喜無限。”\n"NOR);
  ...
  command("sigh");
  command("say 我還是先回去了，爹爹和伯父一定著急了。");
  command("chat*bye"+query("id", me));	
  command("wave"+query("id", me));
  tell_room(environment(me),HIG"\n只見，段譽匆匆忙忙地離開，只是神色似乎很是留戀不捨。\n\n"NOR, ({}));
  set_temp(QUESTDIR1+"see_duanyu1", 1, me);
  destruct(ob);
  return;
}
...
int ask_save()
{
	...
	if (!me->query_condition("killer")
		 ...
		 && query_temp(QUESTDIR1+"kill_duan", me )
		 && query(QUESTDIR1+"wuliangover", me) )
	{
...
set(QUESTDIR1+"over", 1, 		me);//結束
		me->delete_temp("quest/busy");//任務衝突標誌取消
		me->add_busy(4);
		i=10+random(10);
addn("max_neili", i, 		me);
addn("combat_exp", me->query_skill("force")+400, 		me);
addn("mana", me->query_skill("force")/3, 		me);
		...
		log_file("quest/TLBB",sprintf("%s(%s)凌波微步篇完成。獎勵：%d點最大內力，%d點經驗，%d點聲望。\n",me->name(1),query("id", me),i,(me->query_skill("force")+400),me->query_skill("force")/3));
		remove_call_out("dest");
		call_out("dest",5,me,ob);
		return 1;
	}
	if( query(QUESTDIR2+"start", me) && query_temp(QUESTDIR2+"pushdown", me) && !query(QUESTDIR2+"over", me) )
	{
		command("say"+query("name", me)+"已經是第二次相救了，真的非常感激，我這裡正好有塊礦石，你且拿去。");
		obj=new("/d/zhiye/obj/kuangshi1");
		obj->set_level(5+random(3));
		obj->move(me);
		message_vision("$N給了$n一塊"+query("name", obj)+"。\n",ob,me);
    	command("chat*thank"+query("id", me));
    	command("wave"+query("id", me));
    	command("bye"+query("id", me));
		//設定標誌
set(QUESTDIR+"time", time(), 		me);
set(QUESTDIR+"combat_exp",query("combat_exp",  me), 		me);	
set(QUESTDIR2+"over", 1, 		me);//結束	
delete_temp("quest/天龍八部", 		me);
		me->delete_temp("quest/busy");//任務衝突標誌取消
		i=10+random(10);
addn("max_neili", i, 		me);
addn("combat_exp", (me->query_skill("force")+500), 		me);
addn("mana", me->query_skill("force")/3, 		me);
		tell_object(me,HIY"\n你於"+NATURE_D->game_time()+"經過千辛萬苦終於段譽從天龍寺解救出來，獲得"+i+"點最大內力、"+((int)me->query_skill("force")+500)+"點經驗和"+(int)me->query_skill("force")/3+"點聲望。\n\n");
		log_file("quest/TLBB",sprintf("%s(%s)天龍營救篇完成。獎勵：%d點最大內力，%d點經驗，%d點聲望和%s。\n",me->name(1),query("id", me),i,(me->query_skill("force")+500),me->query_skill("force")/3,query("name", obj)));
		destruct(this_object());
		return 1;
	}	
	if( random(2))command("nod"+query("id", me));
	else command("jump"+query("id", me));
	return 1;
}
...
void dest(object me,object ob)
{
  ...
  set("exits/down", "/maze/wuliang/midao1", environment(ob));
