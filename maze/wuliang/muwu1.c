...
void init()
{
	...
	if ( interactive(me)
	 ...
	 && query_temp("quest/天龍八部/凌波微步篇/kill", me)){
		obj=new(__DIR__"npc/dyq");
	  obj->set_name("段延慶", ({ "duan yanqing", "duan","yanqing"}) );
set("title", HIC"四大惡人"NOR, 	obj);
set("nickname", HIG"惡貫滿盈"NOR, 	obj);
...
int valid_leave(object me,string dir)
{
	...
	if ( interactive(me = this_player()) 
	 && query_temp("quest/天龍八部/凌波微步篇/kill", this_player() )
	 && (dir =="north")){
delete_temp("quest/天龍八部/凌波微步篇/kill", 		this_player());
set_temp("quest/天龍八部/凌波微步篇/kill_duan", 1, 		this_player());
		return 1;
	}
